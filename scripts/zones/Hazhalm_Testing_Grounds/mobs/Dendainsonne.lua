...
-----------------------------------
-- Uses standard Behemoth moves
-- Responds to incoming damage with emotes, indicating internal rage state
...
entity.onMobSpawn = function(mob)
    ...
    mob:addListener('TAKE_DAMAGE', 'RAGE_INCREASE', function(mobArg, amount, attacker, attackType, damageType)
        ...
        if rageLevel == 2 then
            if
                mob:getLocalVar('meteorCD') <= os.time() and
                notBusy(mob) and
                mob:getLocalVar('meteorQueued') ~= 1
            then
                mob:messageText(mob, ID.text.DENDAINSONNE_MOCKING_YOU, false)
                mob:setLocalVar('meteorQueued', 1)
                mob:castSpell(218, target) -- Meteor
            else
                mob:messageText(mob, ID.text.DENDAINSONNE_OUT_OF_CONTROL, false)
            end

            mob:setLocalVar('rageLevel', 2)
        else
            mob:messageText(mob, ID.text.DENDAINSONNE_ENRAGED, false)
            mob:setLocalVar('rageLevel', 1)
        end

        mob:setLocalVar('rageIncreaseCD', os.time() + 3)
    end)

    mob:addListener('MAGIC_STATE_EXIT', 'DENDAINSONNE_MAGIC_EXIT', function(mobArg, spell)
        if spell:getID() == 218 then
            mob:setLocalVar('rageLevel', 0)
            mob:setLocalVar('meteorCD', os.time() + 30)
            mob:setLocalVar('meteorQueued', 0)
        end
    end)
end
