...
void CTransportHandler::InitializeTransport(IPP mapIPP)
{
    ...
    const auto query = "SELECT id, transport, door, dock_x, dock_y, dock_z, dock_rot, "
    ...
    FOR_DB_MULTIPLE_RESULTS(rset)
    {
            ...
            uint32 transportId = rset->get<uint32>("transport");
            zoneTown.ship.dock.zone = zoneutils::GetZone((transportId >> 12) & 0x0FFF);
            ...
            zoneTown.ship.npc = zoneutils::GetEntity(transportId, TYPE_SHIP);
            if (!zoneTown.ship.npc)
            {
                ShowError("Transport <%u>: transport not found", rset->get<uint8>("id"));
                continue;
            }
            ...
            zoneTown.ship.timeOffset      = xi::vanadiel_clock::minutes(rset->get<int32>("time_offset"));
            zoneTown.ship.timeInterval    = xi::vanadiel_clock::minutes(rset->get<int32>("time_interval"));
            zoneTown.ship.timeArriveDock  = xi::vanadiel_clock::minutes(rset->get<int32>("time_anim_arrive"));
            zoneTown.ship.timeDepartDock  = zoneTown.ship.timeArriveDock + xi::vanadiel_clock::minutes(rset->get<int32>("time_waiting"));
            zoneTown.ship.timeVoyageStart = zoneTown.ship.timeDepartDock + xi::vanadiel_clock::minutes(rset->get<int32>("time_anim_depart") - 1);
            ...
            {
                ShowError("Transport <%u>: door not found", rset->get<uint8>("id"));
                continue;
            }
            if (zoneTown.ship.timeArriveDock < xi::vanadiel_clock::minutes(10))
            {
                ShowError("Transport <%u>: time_anim_arrive must be > 10", rset->get<uint8>("id"));
                continue;
            }
            if (zoneTown.ship.timeInterval < zoneTown.ship.timeVoyageStart)
            {
                ShowError("Transport <%u>: time_interval must be > time_anim_arrive + time_waiting + time_anim_depart", rset->get<uint8>("id"));
                continue;
            }
    ...
    const auto voyageQuery = "SELECT zone, time_offset, time_interval, time_waiting, time_anim_arrive, time_anim_depart "
                              "FROM transport LEFT JOIN "
                              "zone_settings ON zone = zoneid WHERE "
                              "IF(? <> 0, ? = zoneip AND ? = zoneport, TRUE)";

    const auto voyageRset = db::preparedStmt(voyageQuery, mapIPP.getIP(), mapIPP.getIPString(), mapIPP.getPort());
    
    FOR_DB_MULTIPLE_RESULTS(voyageRset)
    {
        TransportZone_Voyage voyageZone{};

        voyageZone.voyageZone = nullptr;
        voyageZone.voyageZone = zoneutils::GetZone(voyageRset->get<uint8>("zone"));

        if (voyageZone.voyageZone != nullptr && voyageZone.voyageZone->GetID() > 0)
        {
            voyageZone.timeOffset   = xi::vanadiel_clock::minutes(voyageRset->get<int32>("time_offset"));
            voyageZone.timeInterval = xi::vanadiel_clock::minutes(voyageRset->get<int32>("time_interval"));

            voyageZone.timeArriveDock  = xi::vanadiel_clock::minutes(voyageRset->get<int32>("time_anim_arrive"));
            voyageZone.timeDepartDock  = voyageZone.timeArriveDock + xi::vanadiel_clock::minutes(voyageRset->get<int32>("time_waiting"));
            voyageZone.timeVoyageStart = voyageZone.timeDepartDock + xi::vanadiel_clock::minutes(voyageRset->get<int32>("time_anim_depart"));
        ...
        {
            ShowError("TransportZone <%u>: zone not found", voyageRset->get<uint8>("zone"));
        }
    }
}
