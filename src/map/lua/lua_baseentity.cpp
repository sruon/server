...
bool CLuaBaseEntity::delContainerItems(const sol::object& containerID)
{
    ...
    for (uint8 equipmentSlot = 0; equipmentSlot <= 15; equipmentSlot++)
    {
        if (const auto eq = PChar->equipLocation(equipmentSlot); eq && eq->container == location)
        {
...
void CLuaBaseEntity::addShopItem(uint16 itemID, double rawPrice, const sol::object& arg2, const sol::object& arg3)
{
    ...
    PChar->shopState().push(itemID, price, guildID, guildRank);
}
        ...
        if (rset && rset->rowsCount() && rset->next())
        {
            ...
            if (charutils::AddItem(PChar, LOC_INVENTORY, std::move(PItem)) != ERROR_SLOTID)
            {
                ...
                if (equip)
                {
                    ...
                    PChar->bindings().bindEquip(SLOT_LINK2, LOC_INVENTORY, PItemLinkPearl->getSlotID(), PItemLinkPearl);
                    ...
                    PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, PItemLinkPearl, LOC_INVENTORY, PItemLinkPearl->getSlotID());
                    PChar->pushPacket<GP_SERV_COMMAND_ITEM_SAME>(PChar);
...
auto CLuaBaseEntity::dispatchDeclaredTrades(CLuaBaseEntity* npcWrapper, const sol::table& decls) -> bool
{
    ...
    sol::table offers = buildOffersTable(lua, tx);
    ...
    for (const auto& kv : decls)
    {
        ...
        if (eventField.get_type() == sol::type::table)
        {
            ...
            CBaseEntity* npcPtr = npcWrapper ? npcWrapper->GetBaseEntity() : nullptr;
            tx->armPending(eventId, npcPtr, std::move(onFinish));
