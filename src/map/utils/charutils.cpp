...
void LoadInventory(CCharEntity* PChar)
{
    ...
    if (rset && rset->rowsCount())
    {
        while (rset->next())
        {
            auto PItemOwn = itemutils::GetItem(rset->get<uint16>("itemid"));
            if (PItemOwn != nullptr)
            {
                PItemOwn->setLocationID(rset->get<uint8>("location"));
                PItemOwn->setSlotID(rset->get<uint8>("slot"));
                PItemOwn->setQuantity(rset->get<uint32>("quantity"));
                PItemOwn->setCharPrice(rset->get<uint32>("bazaar"));

                db::extractFromBlob(rset, "extra", PItemOwn->m_extra);

                if (PItemOwn->isType(ITEM_LINKSHELL))
                {
                    if (static_cast<CItemLinkshell*>(PItemOwn.get())->GetLSType() == 0)
                    {
                        static_cast<CItemLinkshell*>(PItemOwn.get())->SetLSType((LSTYPE)(PItemOwn->getID() - 0x200));
                    }
                    PItemOwn->setSignature(rset->get<std::string>("signature"));
                }
                else if (PItemOwn->hasFlag(ItemFlag::Inscribable))
                {
                    PItemOwn->setSignature(rset->get<std::string>("signature"));
                }

                if (auto* PItemUsable = dynamic_cast<CItemUsable*>(PItemOwn.get()))
                {
                    uint32 useTime = 0;
                ...
                if (PItemOwn->isType(ITEM_FURNISHING) && (PItemOwn->getLocationID() == LOC_MOGSAFE || PItemOwn->getLocationID() == LOC_MOGSAFE2))
                {
                    if (static_cast<CItemFurnishing*>(PItemOwn.get())->isInstalled())
                    {
                        PChar->getStorage(LOC_STORAGE)->AddBuff(static_cast<CItemFurnishing*>(PItemOwn.get())->getStorage());
                    }
                }

                const uint8  locationId = PItemOwn->getLocationID();
                const uint8  slotId     = PItemOwn->getSlotID();
                const uint32 bazaar     = PItemOwn->getCharPrice();

                ItemStore::placeInInventory(PItemOwn.get(), PChar->id, locationId, slotId);

                CItem* inserted = PItemOwn.get();
                PChar->getStorage(locationId)->InsertItem(std::move(PItemOwn), slotId);

                if (bazaar != 0 && locationId == LOC_INVENTORY)
                {
                    PChar->bindings().addBazaar(LOC_INVENTORY, slotId, inserted);
                }
            }
        }
    }
    ...
    if (rset)
    {
        ...
        for (const auto& [equipSlotId, inventoryLoc] : equipSlotData)
        {
            ...
            else
            {
                ...
                {
                    PChar->bindings().bindEquip(equipSlotId, inventoryLoc.second, inventoryLoc.first, PItem);
        ...
        if (PLinkshell1)
        {
            ...
            { // if the linkshell has been broken, unequip
                ...
                PChar->bindings().clearEquip(SLOT_LINK1);
        ...
        if (PLinkshell2)
        {
            ...
            { // if the linkshell has been broken, unequip
                ...
                PChar->bindings().clearEquip(SLOT_LINK2);
                db::preparedStmt("DELETE char_equip FROM char_equip WHERE charid = ? AND slotid = ? AND containerid = ? LIMIT 1",
                                 PChar->id,
                                 SlotID,
                                 LocationID);
...
void SendInventory(CCharEntity* PChar)
{
    auto pushContainer = [&](auto LocationID)
    {
        ...
        for (uint8 slotID = 0; slotID <= size; ++slotID)
        {
            ...
            if (PItem != nullptr)
            {
                PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, PItem, LocationID, slotID);
            }
        }
    ...
    for (int32 i = 0; i < 16; ++i)
    {
        ...
        if (PItem != nullptr)
        {
            // Reconstruct binding on inventory-resend (login / zone-in).
            if (const auto eq = PChar->equipLocation(i))
            {
                PChar->bindings().bindEquip(i, eq->container, eq->index, PItem);
            }
    ...
    if (PItem != nullptr)
    {
        if (const auto eq = PChar->equipLocation(SLOT_LINK1))
        {
            PChar->bindings().bindEquip(SLOT_LINK1, eq->container, eq->index, PItem);
            PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, PItem, eq->container, eq->index);
        }
    ...
    if (PItem != nullptr)
    {
        if (const auto eq = PChar->equipLocation(SLOT_LINK2))
        {
            PChar->bindings().bindEquip(SLOT_LINK2, eq->container, eq->index, PItem);
            PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, PItem, eq->container, eq->index);
        }
...
auto AddItem(CCharEntity* PChar, uint8 LocationID, uint16 ItemID, uint32 quantity, bool silence) -> uint8
{
...
auto AddItem(CCharEntity* PChar, uint8 LocationID, std::unique_ptr<CItem> PItem, bool silence) -> uint8
{
    ...
    // Keep a non-owning read handle for post-insert packet/DB work.
    // Ownership transfers into the container on a successful insert;
    // on failure the container auto-drops and rawItem dangles — guarded
    // by the ERROR_SLOTID branch.
    CItem* rawItem = PItem.get();
    uint8  SlotID  = PChar->getStorage(LocationID)->InsertItem(std::move(PItem));

    if (SlotID != ERROR_SLOTID)
    {
        // Register the new item's owner before any DB I/O so the
        // ItemStore invariant (no unowned items in a container) holds
        // from the moment of insertion.
        ItemStore::placeInInventory(rawItem, PChar->id, LocationID, SlotID);
        ...
        if (!db::preparedStmt(Query, PChar->id, LocationID, SlotID, rawItem->getID(), rawItem->getQuantity(), rawItem->getSignature(), rawItem->m_extra))
        {
            ShowError("AddItem: Cannot insert item to database");
            // Container auto-drops the displaced item when we overwrite
            // with an empty unique_ptr.
            PChar->getStorage(LocationID)->InsertItem(nullptr, SlotID);
            return ERROR_SLOTID;
        }

        PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, rawItem, static_cast<CONTAINER_ID>(LocationID), SlotID);
        PChar->pushPacket<GP_SERV_COMMAND_ITEM_SAME>(PChar);
    }
    else
    {
...
uint8 MoveItem(CCharEntity* PChar, uint8 LocationID, uint8 SlotID, uint8 NewSlotID)
{
    ...
    if (PItemContainer->GetFreeSlotsCount() != 0)
    {
        ...
        auto item = PItemContainer->ReleaseItem(SlotID);
        if (item == nullptr)
        {
            ShowError("charutils::MoveItem: item can't be moved");
            return ERROR_SLOTID;
        ...
        NewSlotID = (NewSlotID == ERROR_SLOTID)
                        ? PItemContainer->InsertItem(std::move(item))
                        : PItemContainer->InsertItem(std::move(item), NewSlotID);
        ...
        if (NewSlotID != ERROR_SLOTID)
        {
            ...
            {
                // Re-stamp the owner to the new slot so ItemStore's
                // view of (charId, location, slot) matches the
                // container.
                ItemStore::reslotInventory(PItemContainer->GetItem(NewSlotID), LocationID, NewSlotID);

                PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, nullptr, static_cast<CONTAINER_ID>(LocationID), SlotID, PItemContainer->GetItem(NewSlotID));
                PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, PItemContainer->GetItem(NewSlotID), static_cast<CONTAINER_ID>(LocationID), NewSlotID);
                return NewSlotID;
            }

            // DB rollback — put the item back in its original slot.
            auto rollback = PItemContainer->ReleaseItem(NewSlotID);
            PItemContainer->InsertItem(std::move(rollback), SlotID);
        }
    }
    ShowError("charutils::MoveItem: item can't be moved");
    return ERROR_SLOTID;
...
uint32 UpdateItem(CCharEntity* PChar, uint8 LocationID, uint8 slotID, int32 quantity, BypassItemStateCheck bypass)
{
    CItem* PItem = PChar->getStorage(LocationID)->GetItem(slotID);
    ...
    if (PItem == nullptr)
    {
        ...
        PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, nullptr, static_cast<CONTAINER_ID>(LocationID), slotID);
        return 0;
    }
    ...
    if (PState)
    {
        ...
        if (item && item->getSlotID() == PItem->getSlotID() && item->getLocationID() == PItem->getLocationID() && !bypass)
        {
            return 0;
        }
    }
    ...
    else if (newQuantity == 0)
    {
        db::preparedStmt("DELETE FROM char_inventory "
                         ...
                         LocationID,
                         slotID);
        // Pop the item out; the local unique_ptr owns it for the rest
        // of this block, then auto-drops at scope exit.
        auto droppedItem = PChar->getStorage(LocationID)->ReleaseItem(slotID);
        PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, nullptr, static_cast<CONTAINER_ID>(LocationID), slotID);
        ...
        // droppedItem auto-drops at scope exit.
    }
    return ItemID;
}
...
void UnequipItem(CCharEntity* PChar, uint8 equipSlotID, Recalculate recalculate)
{
    ...
    if ((PItem != nullptr) && PItem->isType(ITEM_EQUIPMENT))
    {
        ...
        PChar->bindings().clearEquip(equipSlotID);
        ...
        switch (equipSlotID)
        {
            ...
            case SLOT_AMMO:
            {
                if (!PChar->equipLocation(SLOT_RANGED))
                {
                    PChar->look.ranged = 0;
                }
...
bool EquipArmor(CCharEntity* PChar, uint8 slotID, uint8 equipSlotID, uint8 containerID)
{
    ...
    if (PItem->getEquipSlotId() & (1 << equipSlotID))
    {
        ...
        PChar->bindings().bindEquip(equipSlotID, containerID, slotID, PItem);
...
void UpdateRemovedSlotsLookForLockStyle(CCharEntity* PChar)
{
    ...
    for (auto i = 0; i < 16; i++)
    {
        ...
        // Read-only lookup from the template table — no fresh clone.
        auto* PItem = dynamic_cast<CItemEquipment*>(itemutils::GetItemPointer(items[i]));
        if (!PItem)
        {
            continue;
        }
...
void AddItemToRecycleBin(CCharEntity* PChar, uint32 container, uint8 slotID, uint8 quantity)
{
    ...
    if (OtherContainer->GetItem(slotID) == nullptr)
    {
        return;
    }
    ...
    {
        // Release from source and hand to recycle bin.
        auto   item      = OtherContainer->ReleaseItem(slotID);
        CItem* rawItem   = item.get();
        uint8  NewSlotID = RecycleBin->InsertItem(std::move(item));
        ...
        {
            // Send update packets
            PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, nullptr, static_cast<CONTAINER_ID>(container), slotID);
            PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, rawItem, LOC_RECYCLEBIN, NewSlotID);
            PChar->pushPacket<GP_SERV_COMMAND_MESSAGE>(nullptr, rawItem->getID(), quantity, MsgStd::ThrowAway);
            luautils::OnItemDrop(PChar, rawItem, IsRecycleBin::Yes);
        }
        else
        {
            // Move not successful, put things back how they were
            auto restore = RecycleBin->ReleaseItem(NewSlotID);
            OtherContainer->InsertItem(std::move(restore), slotID);
        }
    }
    else // Bin is full
    {
        // Evict recycle bin slot 1 (auto-drop via release + scope exit).
        if (CItem* PEvictedItem = RecycleBin->GetItem(1); PEvictedItem != nullptr)
        {
            luautils::OnItemDrop(PChar, PEvictedItem);
        }
        auto evicted = RecycleBin->ReleaseItem(1);
        db::preparedStmt("DELETE FROM char_inventory WHERE charid = ? AND location = ? AND slot = ? LIMIT 1",
                         PChar->id,
                         LOC_RECYCLEBIN,
                         1);
        // evicted goes out of scope below → dropped.

        // Shift recycle bin slots 2-10 down to 1-9.
        for (int i = 2; i <= 10; ++i)
        {
            auto moving = RecycleBin->ReleaseItem(i);
            if (moving == nullptr)
            {
                continue;
            }

            const std::string movingName = moving->getName();
            RecycleBin->InsertItem(std::move(moving), i - 1);
            ...
            if (!rset || !rset->rowsAffected())
            {
                ShowError("Problem moving Recycle Bin items! (%s - %s)", PChar->getName(), movingName.c_str());
            }
        }

        // Move item from original container to recycle bin slot 10.
        auto   sourceItem = OtherContainer->ReleaseItem(slotID);
        CItem* rawItem    = sourceItem.get();
        RecycleBin->InsertItem(std::move(sourceItem), 10);
        ...
        if (!rset || !rset->rowsAffected())
        {
            ShowError("Problem moving Recycle Bin items! (%s - %s)", PChar->getName(), rawItem->getName());
        }
        ...
        for (int i = 1; i <= 10; ++i)
        {
            ...
            PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, PUpdatedItem, LOC_RECYCLEBIN, i);
        }
        PChar->pushPacket<GP_SERV_COMMAND_MESSAGE>(nullptr, rawItem->getID(), quantity, MsgStd::ThrowAway);
        luautils::OnItemDrop(PChar, rawItem, IsRecycleBin::Yes);
    }
...
void EquipItem(CCharEntity* PChar, uint8 slotID, uint8 equipSlotID, uint8 containerID)
{
    ...
    else
    {
        if ((PItem != nullptr) && PItem->isType(ITEM_EQUIPMENT))
        {
            if (!ItemStore::isBusy(PChar, PItem) && EquipArmor(PChar, slotID, equipSlotID, containerID))
            {
                ...
                if (PItem->isType(ITEM_USABLE) && ((CItemUsable*)PItem)->getCurrentCharges() != 0)
                {
                    ...
                    PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, PItem, static_cast<CONTAINER_ID>(containerID), slotID);
                }
                // Binding is already set by EquipArmor via setEquip;
...
void SaveCharEquip(CCharEntity* PChar)
{
    ...
    {
        const auto eq = PChar->equipLocation(i);
        if (!eq)
        {
...
void SendToZone(CCharEntity* PChar, uint16 zoneId)
{
    ...
    if (PChar->craftState.isSynthing())
    {
...
void forceSynthCritFail(const std::string& sourceFunction, CCharEntity* PChar)
{
    ...
    PChar->craftState.clean(); // Clean to reset m_ItemCount to 0
}
...
void removeCharFromZone(CCharEntity* PChar)
{
    ...
    // Cancel any live PlayerTradeTransaction before tradePending gets cleared.
    // activePlayerTradeTx resolves through tradePending so we must find-then-
    // clean BEFORE clearing it. PlayerTradeTransaction::abort handles the
    // counterparty notify + both-sides tradePending cleanup.
