...
auto LoadChar(Scheduler& scheduler, MapConfig config, const uint32 charId) -> std::unique_ptr<CCharEntity>
{
    ...
    if (rset && rset->rowsCount() && rset->next())
    {
        PChar->SetMJob(rset->get<xi::Job>("mjob"));
        PChar->SetSJob(rset->get<xi::Job>("sjob"));
...
void SaveJobChangeGear(CCharEntity* PChar)
{
    ...
    db::preparedStmt(
        ...
        "back = VALUES(back)",
        PChar->id,
        static_cast<uint8>(PChar->GetMJob()),
        main,
        sub,
        ranged,
        ammo,
...
int16 ArtsBonusSkill(CCharEntity* PChar, SKILLTYPE SkillID)
{
    ...
    uint16 artsSkill    = battleutils::GetMaxSkill(SKILL_ENHANCING_MAGIC, xi::Job::RDM, PChar->GetMLevel());                          // B+ skill
    uint16 skillCapD    = battleutils::GetMaxSkill(SkillID, xi::Job::SCH, PChar->GetMLevel());                                        // D skill cap
    uint16 skillCapE    = battleutils::GetMaxSkill(SKILL_DARK_MAGIC, xi::Job::RDM, PChar->GetMLevel());                               // E skill cap
...
void SaveCharStats(CCharEntity* PChar)
{
    ...
    db::preparedStmt("UPDATE char_stats "
                     ...
                     static_cast<uint8>(PChar->GetMJob()),
                     static_cast<uint8>(PChar->GetSJob(true)),
...
void SaveCharJob(const CCharEntity* PChar, const xi::Job job)
{
    ...
    {
        ShowWarningFmt("Attempt to save Invalid Job with xi::Job {}.", static_cast<uint16>(job));
        return;
    }
...
void SaveCharExp(const CCharEntity* PChar, const xi::Job job)
{
    ...
    if (job == xi::Job::NONE || static_cast<uint8>(job) >= MAX_JOBTYPE)
    {
        ShowWarningFmt("Attempt to save Char XP with invalid xi::Job {}.", static_cast<uint16>(job));
        return;
    }
...
uint16 getWideScanRange(xi::Job job, uint8 level)
{
    // Set Widescan range
