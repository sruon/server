...
void auctionutils::SellingItems(CCharEntity* PChar, GP_AUC_PARAM_ASKCOMMIT param)
{
    ...
    if (PItem->getID() == param.ItemNo && !ItemStore::isBusy(PItem) && !PItem->hasFlag(ItemFlag::NoAuction))
    {
...
void auctionutils::ProofOfPurchase(CCharEntity* PChar, GP_AUC_PARAM_LOT param)
{
    ...
    if (PItem && !ItemStore::isBusy(PItem) && !PItem->hasFlag(ItemFlag::NoAuction) && PItem->getQuantity() >= param.ItemStacks)
    {
        if (isPartiallyUsed(PItem))
        {
        ...
        if (PGil->getQuantity() < auctionFee || ItemStore::isBusy(PGil))
        {
...
auto auctionutils::PurchasingItems(CCharEntity* PChar, GP_AUC_PARAM_BID param) -> bool
{
    ...
    else
    {
        ...
        if (PItem != nullptr)
        {
            ...
            if (gil != nullptr && gil->isType(ITEM_CURRENCY) && gil->getQuantity() >= param.BidPrice && !ItemStore::isBusy(gil))
            {
