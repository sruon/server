...
namespace mobutils
{
    ...
    */
    ...
    {
        // load family mods
        const auto familyModsQuery = "SELECT familyid, modid, value, is_mob_mod FROM mob_family_mods";
        const auto familyModsRset = db::preparedStmt(familyModsQuery);

        FOR_DB_MULTIPLE_RESULTS(familyModsRset)
        {
            ModsList_t* familyMods = GetMobFamilyMods(familyModsRset->get<uint32>("familyid"), true);

            CModifier* mod = new CModifier(static_cast<Mod>(familyModsRset->get<int32>("modid")));
            mod->setModAmount(familyModsRset->get<int32>("value"));

            int8 isMobMod = familyModsRset->get<int8>("is_mob_mod");
            if (isMobMod == 1)
            {
        ...
        // load pool mods
        const auto poolModsQuery = "SELECT poolid, modid, value, is_mob_mod FROM mob_pool_mods";
        const auto poolModsRset = db::preparedStmt(poolModsQuery);

        FOR_DB_MULTIPLE_RESULTS(poolModsRset)
        {
            uint16      pool     = poolModsRset->get<uint16>("poolid");
            ModsList_t* poolMods = GetMobPoolMods(pool, true);

            Mod id = static_cast<Mod>(poolModsRset->get<int32>("modid"));

            CModifier* mod = new CModifier(id);
            mod->setModAmount(poolModsRset->get<uint32>("value"));

            int8 isMobMod = poolModsRset->get<int8>("is_mob_mod");
            if (isMobMod == 1)
            {
    ...
    {
        const auto query = "SELECT zoneid, mob_groups.name, packet_name, "
        ...
        const auto rset = db::preparedStmt(query, groupid, zoneID);
        ...
        FOR_DB_SINGLE_RESULT(rset)
        {
            ...
            PMob->m_Aggro         = rset->get<uint32>("aggro");
    ...
    CMobEntity* InstantiateDynamicMob(uint32 groupid, uint16 groupZoneId, uint16 targetZoneId)
    {
        ...
        const auto query = "SELECT zoneid, mob_groups.name, packet_name, "
                           ...
                           "fire_res_rank, ice_res_rank, wind_res_rank, earth_res_rank, lightning_res_rank, water_res_rank, light_res_rank, dark_res_rank, "
                           "Element, mob_pools.familyid, name_prefix, entityFlags, animationsub, "
                           "(mob_family_system.HP / 100) AS family_hp, (mob_family_system.MP / 100) AS family_mp, hasSpellScript, spellList, mob_groups.poolid, "
                           "allegiance, namevis, aggro, mob_pools.skill_list_id, mob_pools.true_detection, mob_family_system.detects "
                           "FROM mob_groups INNER JOIN mob_pools ON mob_groups.poolid = mob_pools.poolid "
                           "INNER JOIN mob_resistances ON mob_pools.resist_id = mob_resistances.resist_id "
                           "INNER JOIN mob_family_system ON mob_pools.familyid = mob_family_system.familyID "
        ...
        const auto rset = db::preparedStmt(query, groupid, groupZoneId);
        ...
        FOR_DB_SINGLE_RESULT(rset)
        {
            ...
            uint16 sqlModelID[10];
            auto modelBlob = rset->get<std::string>("modelid");
            std::memcpy(&sqlModelID, modelBlob.data(), 20);
            PMob->look = look_t(sqlModelID);
            ...
            // Setup HP / MP Stat Percentage Boost
            PMob->HPscale = rset->get<float>("family_hp");
            PMob->MPscale = rset->get<float>("family_mp");
