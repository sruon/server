...
namespace petutils
{
    ...
    void LoadPetList()
    {
        ...
        FOR_DB_MULTIPLE_RESULTS(rset)
        {
                ...
                Pet->radius    = rset->get<uint32>("mobradius");
                Pet->EcoSystem = static_cast<ECOSYSTEM>(rset->get<int32>("ecosystemID"));
                ...
                Pet->HPscale = rset->get<float>("(mob_family_system.HP / 100)");
                Pet->MPscale = rset->get<float>("(mob_family_system.MP / 100)");
                ...
                Pet->m_dmgType      = static_cast<DAMAGE_TYPE>(rset->get<int32>("damageType"));
    ...
    */
    ...
    void LoadPet(CBattleEntity* PMaster, uint32 PetID, bool spawningFromZone)
    {
        ...
        else if (PetID == PETID_WYVERN)
        {
            ...
            FOR_DB_MULTIPLE_RESULTS(rset)
            {
                uint16 wyvernid = rset->get<uint16>("wyvernid");
        ...
        else if (PetID == PETID_CHOCOBO)
        {
            ...
            const auto chocoboQuery = "SELECT chocoboid FROM char_pet WHERE charid = ?";
            const auto chocoboRset = db::preparedStmt(chocoboQuery, PMaster->id);

            FOR_DB_MULTIPLE_RESULTS(chocoboRset)
            {
                uint32 chocoboid = chocoboRset->get<uint32>("chocoboid");
                ...
                if (chocoboid != 0)
                {
                    ...
                    const auto nameQuery = "SELECT name FROM pet_name WHERE id = ? OR id = ?";
                    const auto nameRset = db::preparedStmt(nameQuery, chocoboname1, chocoboname2);

                    FOR_DB_MULTIPLE_RESULTS(nameRset)
                    {
                        if (chocoboname1 != 0 && chocoboname2 != 0)
                        {
                            PPetData->name.insert(0, nameRset->get<std::string>("name"));
                        }
                    }
                }
            }
