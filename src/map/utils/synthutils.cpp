...
auto isRightRecipe(CCharEntity* PChar) -> bool
{
    ...
    const auto ingredient1 = PChar->craftState.ingredients[0].itemId;
    ...
    if (synthRecipes.contains(possibleRecipeKey))
    {
        ...
        if (recipe.RequiredKeyItem == KeyItem::NONE || charutils::hasKeyItem(PChar, recipe.RequiredKeyItem))
        {
            ...
            PChar->craftState.craftMode  = recipe.Desynth;
...
void handleMaterialLoss(CCharEntity* PChar)
{
    ...
    // Loop variables
    uint8 invSlotID  = PChar->craftState.ingredients[0].invSlot;
    uint8 nextSlotID = 0;
    uint8 lostCount  = 0;
    uint8 totalCount = 0;
    uint8 random     = 0;
    ...
    for (uint8 slotID = 1; slotID <= 8; ++slotID)
    {
        ...
        if (invSlotID != nextSlotID)
        {
            ...
            if (PItem != nullptr)
            {
                ...
                if (lostCount > 0)
                {
                    charutils::UpdateItem(PChar, LOC_INVENTORY, invSlotID, -(int32)lostCount, charutils::BypassItemStateCheck::Yes);
                    lostCount = 0;
                }
                else
                {
...
void handleSynthSuccess(CCharEntity* PChar)
{
    ...
    invSlotID = PChar->craftState.ingredients[0].invSlot;
    ...
    for (uint8 slotID = 1; slotID <= 8; ++slotID)
    {
        ...
        if (invSlotID != nextSlotID)
        {
            if (invSlotID != 0xFF)
            {
                ...
                if (PItem != nullptr)
                {
                    charutils::UpdateItem(PChar, LOC_INVENTORY, invSlotID, -(int32)removeCount, charutils::BypassItemStateCheck::Yes);
                }
            }
            invSlotID   = nextSlotID;
            nextSlotID  = 0;
    ...
    if (PItem != nullptr)
    {
        ...
        PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, PItem, LOC_INVENTORY, invSlotID);
    }
