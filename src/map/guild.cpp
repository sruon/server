...
auto CGuild::addGuildPoints(CCharEntity* PChar, const CItem* PItem, const uint16 offeredQty) const -> std::pair<uint8, int16>
{
    ...
    if (PItem)
    {
        for (auto& GPItem : m_GPItems[rank - 3])
        {
            if (GPItem.item->getID() == PItem->getID())
            {
                ...
                uint16 quantity    = std::min<uint16>((((GPItem.maxpoints - std::clamp<uint16>(curPoints, 0, GPItem.maxpoints)) / GPItem.points) + 1), offeredQty);
