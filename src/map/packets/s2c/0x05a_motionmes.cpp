...
GP_SERV_COMMAND_MOTIONMES::GP_SERV_COMMAND_MOTIONMES(const CCharEntity* PChar, const uint32 targetId, const uint16 targetIndex, Emote emoteId, EmoteMode emoteMode, const uint16 extra)
{
    ...
    {
        const auto* PWeapon = PChar->getStorage(PChar->containerIdFor(SLOT_MAIN))->GetItem(PChar->inventorySlotFor(SLOT_MAIN));
    ...
    {
        packet.Param               = 65535;
        const CItemWeapon* PWeapon = static_cast<CItemWeapon*>(PChar->getStorage(PChar->containerIdFor(SLOT_RANGED))->GetItem(PChar->inventorySlotFor(SLOT_RANGED)));
        if (PWeapon && PWeapon->getID() != 65535)
        {
            ...
            {
                const CItemWeapon* PAmmo = static_cast<CItemWeapon*>(PChar->getStorage(PChar->containerIdFor(SLOT_AMMO))->GetItem(PChar->inventorySlotFor(SLOT_AMMO)));
