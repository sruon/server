...
struct sendflags_t
{
    ...
    sendflags_t& operator|=(const sendflags_t& other)
    {
        *reinterpret_cast<uint8_t*>(this) |= *reinterpret_cast<const uint8_t*>(&other);
        return *this;
    }

    static sendflags_t spawn()
    {
    ...
    static sendflags_t fromUpdateMask(uint8_t mask)
    {
        sendflags_t flags{};
        if (mask & 0x01) flags.Position = 1;
        if (mask & 0x02) flags.ClaimStatus = 1;
        if (mask & 0x04) flags.General = 1;
        if (mask & 0x08) flags.Name = 1;
        if (mask & 0x10) flags.Model = 1;
        if (mask & 0x20) flags.Despawn = 1;
        return flags;
    }
};
...
// This packet is sent by the server to update other player entities information. (Spawn, despawn, general updates etc.)
class GP_SERV_COMMAND_CHAR_PC final : public GP_SERV_PACKET<PacketS2C::GP_SERV_COMMAND_CHAR_PC, GP_SERV_COMMAND_CHAR_PC>
{
public:
    struct PacketData
    {
    ...
    GP_SERV_COMMAND_CHAR_PC(const CCharEntity* PChar);
    GP_SERV_COMMAND_CHAR_PC(sendflags_t SendFlg, CCharEntity* PChar);
};
