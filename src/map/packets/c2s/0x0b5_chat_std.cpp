...
namespace
{
    ...
    {
        // clang-format off
        Async::getInstance()->submit([name = PChar->getName(), zoneId = PChar->getZone(), chatType, rawMessage]()
        {
...
{
    const auto rawMessage                 = asStringFromUntrustedSource(Str, sizeof(Str));
    ...
    switch (static_cast<GP_CLI_COMMAND_CHAT_STD_KIND>(Kind))
    {
        ...
        {
            // Not implemented
            if (settings::get<bool>("map.AUDIT_CHAT") && settings::get<uint8>("map.AUDIT_BALLISTA"))
            {
        ...
        {
            // Not implemented
            if (settings::get<bool>("map.AUDIT_CHAT") && settings::get<uint8>("map.AUDIT_ASSISTJ"))
            {
        ...
        {
            // Not implemented
            if (settings::get<bool>("map.AUDIT_CHAT") && settings::get<uint8>("map.AUDIT_ASSISTE"))
            {
