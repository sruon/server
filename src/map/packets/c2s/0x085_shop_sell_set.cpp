    ...
    const auto auditSale = [](CCharEntity* PChar, uint32_t itemId, uint32_t quantity, uint32_t basePrice)
    {
        if (settings::get<bool>("map.AUDIT_PLAYER_VENDOR"))
        {
            ...
            Async::getInstance()->submit([&]()
            {
                ...
                auto saleTime = earth_time::timestamp();
                ...
                {
                    ShowErrorFmt("Failed to log vendor sale (item: {}, quantity: {}, seller: {}, totalprice: {}, time: {})", itemId, quantity, seller, totalPrice, saleTime);
                }
            });
            // clang-format on
        }
...
auto GP_CLI_COMMAND_SHOP_SELL_SET::validate(MapSession* PSession, const CCharEntity* PChar) const -> PacketValidationResult
{
    return PacketValidator()
        .isNotCrafting(PChar)
        .mustEqual(SellFlag, 1, "SellFlag not equal to 1");
}
