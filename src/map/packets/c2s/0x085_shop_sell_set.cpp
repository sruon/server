...
{
    const auto& pending  = PChar->shopDisplay.pendingSell();
    ...
    if (ItemStore::isBusy(PItem)) // Possible exploit
    {
    ...
    // The isBusy check above already covers tx custody + bindings;
    // no separate m_reserve gate needed any more.
    ...
    PChar->shopDisplay.clearPendingSell();
}
