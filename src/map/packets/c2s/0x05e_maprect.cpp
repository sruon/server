...
#include "map/packets/message_system.h"
#include "map/packets/position.h"
            ...
            if (MyRoomExitMode != 0)
            {
                switch (static_cast<GP_CLI_COMMAND_MAPRECT_MYROOMEXITBIT>(MyRoomExitBit))
                {
                    ...
                    case GP_CLI_COMMAND_MAPRECT_MYROOMEXITBIT::Adoulin:
                        // TODO: Figure out exit handling for Adoulin
                        break;
                ...
                if (MyRoomExitMode == 127)
                {
                    destinationZone = ZONE_MOG_GARDEN;
                }
                else if (MyRoomExitMode == 126) // Go to first floor from second
                {
                    destinationZone = PChar->getZone();
                }
                else if (MyRoomExitMode == 125) // Go to second floor from first
                {
                    destinationZone = PChar->getZone();
                }
            }
