...
auto GP_CLI_COMMAND_ITEM_STACK::validate(MapSession* PSession, const CCharEntity* PChar) const -> PacketValidationResult
{
    return PacketValidator()
        .range("Category", Category, 0, CONTAINER_ID::MAX_CONTAINER_ID - 1);
}

void GP_CLI_COMMAND_ITEM_STACK::process(MapSession* PSession, CCharEntity* PChar) const
{
    const uint8 container = static_cast<uint8>(Category);
            ...
            for (uint8 slotID2 = slotID + 1; slotID2 <= size; ++slotID2)
            {
                    ...
                    !PItem2->isSubType(ITEM_LOCKED) && (PItem2->getReserve() == 0))
                {
                    ...
                    if (moveQty > 0)
                    {
                        ...
                        charutils::UpdateItem(PChar, static_cast<uint8>(PItemContainer->GetID()), slotID2, -(int32)moveQty);
                    }
                }
            }
        }
