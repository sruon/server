...
const auto isValidMovement = [](const CCharEntity* PChar, const CONTAINER_ID from, const CONTAINER_ID to, const uint16_t itemIndex) -> bool
{
    ...
    if (!PItem || ItemStore::isBusy(PChar, PItem) || PItem->getID() == ITEMID::GIL)
    {
        return false;
    }
...
void GP_CLI_COMMAND_ITEM_MOVE::process(MapSession* PSession, CCharEntity* PChar) const
{
    ...
    else // move stack / combine items into stack
    {
        if (this->ItemIndex2 < 82) // 80 + 1
        {
            ...
            if (!PItem2 || PItem2->getID() != PItem->getID() || ItemStore::isBusy(PChar, PItem2))
            {
        ...
        if (PChar->getStorage(this->Category2)->GetFreeSlotsCount() > 0)
        {
            ...
            {
                PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, nullptr, static_cast<CONTAINER_ID>(this->Category1), this->ItemIndex1, rawItem);
                PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, rawItem, static_cast<CONTAINER_ID>(this->Category2), newSlotId);
            }
            else
            {
        ...
        else
        {
            ...
            for (uint8 slotID = 0; slotID <= size; ++slotID)
            {
                ...
                {
                    PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, PSlotItem, static_cast<CONTAINER_ID>(this->Category2), slotID);
                }
            }
