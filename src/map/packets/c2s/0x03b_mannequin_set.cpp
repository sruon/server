...
namespace
{
    const std::set validMannequinContainers = {
        LOC_MOGSAFE,
        LOC_MOGSAFE2,
    };

    // Build Mannequin model id list
    auto getModelIdFromStorageSlot = [](CCharEntity* PChar, const uint8 slot) -> uint16
    {
...
auto GP_CLI_COMMAND_MANNEQUIN_SET::validate(MapSession* PSession, const CCharEntity* PChar) const -> PacketValidationResult
{
    return PacketValidator()
        .oneOf("Category1", static_cast<CONTAINER_ID>(Category1), validMannequinContainers)
        .oneOf<GP_CLI_COMMAND_MANNEQUIN_SET_CONTAINERINDEX>(ContainerIndex)
        .oneOf<GP_CLI_COMMAND_MANNEQUIN_SET_KIND>(Kind);
}

void GP_CLI_COMMAND_MANNEQUIN_SET::process(MapSession* PSession, CCharEntity* PChar) const
{
    auto* PMannequin = PChar->getStorage(Category1)->GetItem(ItemIndex1);
    if (PMannequin == nullptr)
    {
        ShowWarning("GP_CLI_COMMAND_MANNEQUIN_SET: Unable to load mannequin from slot %u in location %u by %s", ItemIndex1, Category1, PChar->getName());
        return;
    }

    switch (static_cast<GP_CLI_COMMAND_MANNEQUIN_SET_KIND>(Kind))
    {
        case GP_CLI_COMMAND_MANNEQUIN_SET_KIND::Equip:
        {
            if (Category2 != LOC_STORAGE) // Only valid for direct equip/unequip
            {
                ShowWarning("GP_CLI_COMMAND_MANNEQUIN_SET: Invalid item location passed to Mannequin Equip packet %u by %s", Category2, PChar->getName());
                return;
            }
            ...
            {
                setStatusOfStorageItemAtSlot(PChar, ItemIndex2, INV_NORMAL);
                PMannequin->m_extra[10 + ContainerIndex] = 0;
            }
            else // Regular Logic
            {
                setStatusOfStorageItemAtSlot(PChar, ItemIndex2, INV_MANNEQUIN);
                PMannequin->m_extra[10 + ContainerIndex] = ItemIndex2;
            }
        }
        break;

        case GP_CLI_COMMAND_MANNEQUIN_SET_KIND::Unequip:
        {
            setStatusOfStorageItemAtSlot(PChar, ItemIndex2, INV_NORMAL);
            PMannequin->m_extra[10 + ContainerIndex] = 0;
        }
        break;
        case GP_CLI_COMMAND_MANNEQUIN_SET_KIND::UnequipAll:
        {
            for (uint8 i = 0; i < 8; ++i)
            {
    ...
    {
        ShowError("GP_CLI_COMMAND_MANNEQUIN_SET: Problem writing Mannequin to database!");
    }
}
