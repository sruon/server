...
void GP_CLI_COMMAND_SHOP_BUY::process(MapSession* PSession, CCharEntity* PChar) const
{
    ...
    if (this->ShopItemIndex >= PChar->shopState().itemsCount())
    {
        ShowError("User '%s' attempting to buy vendor item from an invalid slot!", PChar->getName());
        return;
    }

    const auto&  entry  = PChar->shopState().entry(this->ShopItemIndex);
    ...
    if (!gil || !gil->isType(ITEM_CURRENCY) || ItemStore::isBusy(PChar, gil))
    {
