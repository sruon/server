...
void GP_CLI_COMMAND_ITEM_USE::process(MapSession* PSession, CCharEntity* PChar) const
{
    ...
    auto isEquipped = [&]() -> bool
    {
        ...
        {
            if (const auto eq = PChar->equipLocation(slot); eq && eq->container == this->Category && eq->index == this->PropertyItemIndex)
            {
                return true;
            }
        }
    ...
    // An equip binding on a charged item (Warp Cudgel etc.) is its
    // "equipped" marker — allow use. Anything else busy (tx custody,
    // furniture placement, bazaar listing) → reject.
    ...
    const bool busyAndNotOurEq = ItemStore::isBusy(PChar, PItem) && !isOurEquipped;
    if (busyAndNotOurEq)
    {
        ShowWarningFmt("GP_CLI_COMMAND_ITEM_USE: {} trying to use busy item", PChar->getName());
        return;
    }
