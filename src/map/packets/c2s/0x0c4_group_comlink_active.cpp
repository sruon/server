...
const auto createLinkshell = [](CCharEntity* PChar, CItemLinkshell* PItemLinkshell, const GP_CLI_COMMAND_GROUP_COMLINK_ACTIVE& data)
{
    ...
    if (linkshellId != 0)
    {
        ...
        {
            PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, PNewLinkshell, static_cast<CONTAINER_ID>(data.Category), data.ItemIndex);
        }
    }
    else
    {
...
const auto equipLinkshell = [](CCharEntity* PChar, CItemLinkshell* PItemLinkshell, const GP_CLI_COMMAND_GROUP_COMLINK_ACTIVE& data)
{
    ...
    if (rset && rset->rowsCount() && rset->next() && rset->get<uint8>("broken") == 1)
    {
        ...
        PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, PItemLinkshell, static_cast<CONTAINER_ID>(PItemLinkshell->getLocationID()), PItemLinkshell->getSlotID());
    ...
    if (oldLinkshell)
    {
        ...
        if (POldItemLinkshell && POldItemLinkshell->isType(ITEM_LINKSHELL))
        {
            ...
            PChar->bindings().clearEquip(slot);
    ...
    PChar->bindings().bindEquip(lsEquipSlot, data.Category, data.ItemIndex, PItemLinkshell);
    ...
    PChar->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(PChar, PItemLinkshell, static_cast<CONTAINER_ID>(data.Category), data.ItemIndex);
};
...
const auto unequipLinkshell = [](CCharEntity* PChar, CItemLinkshell* PItemLinkshell, const GP_CLI_COMMAND_GROUP_COMLINK_ACTIVE& data)
{
    ...
    PChar->bindings().clearEquip(SLOT_BACK + data.LinkshellId);
    if (data.LinkshellId == 1)
    {
        PChar->updatemask |= UPDATE_HP;
    }
