...
auto CStatusEffectContainer::BuildPersistRows(bool logout) -> std::vector<PersistedEffect>
{
...
void CStatusEffectContainer::DropEffectsForTransition(bool logout)
{
...
void effects::SaveEffectRows(const std::vector<uint32>& replaceFor, const std::vector<PersistedEffect>& rows)
{
    ...
    db::replaceRows("char_effects", "charid", { "charid", "effectid", "icon", "power", "tick", "duration", "subid", "subpower", "tier", "flags", "timestamp", "sourcetype", "sourcetypeparam", "originid" }, kPersistBatchRows, replaceFor, rows, [](std::vector<db::BoundValue>& params, const PersistedEffect& row)
                    {
                        db::bindValues(params, row.charid, row.effectId, row.icon, row.power, row.tick, row.duration, row.subId, row.subPower, row.tier, row.flags, row.timestamp, row.sourceType, row.sourceTypeParam, row.originId);
                    });
}
