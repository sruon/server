...
void CZone::LoadZoneLines()
{
    ...
    const auto query = "SELECT zoneline, tozone, tox, toy, toz, rotation FROM zonelines WHERE fromzone = ?";
...
void CZone::LoadZoneWeather()
{
    ...
    const auto query = "SELECT weather FROM zone_weather WHERE zone = ?";
    const auto rset  = db::preparedStmt(query, m_zoneID);
    ...
    FOR_DB_SINGLE_RESULT(rset)
    {
        ...
        for (uint16 i = 0; i < WEATHER_CYCLE; i++)
        {
            if (weatherBlob[i])
            {
                const uint8 w_normal = static_cast<uint8>(weatherBlob[i] >> 10);
                const uint8 w_common = static_cast<uint8>((weatherBlob[i] >> 5) & 0x1F);
                const uint8 w_rare   = static_cast<uint8>(weatherBlob[i] & 0x1F);
...
void CZone::LoadZoneSettings()
{
    ...
    FOR_DB_SINGLE_RESULT(rset)
    {
        ...
        m_zoneIP = str2ip(rset->get<std::string>("zoneip").c_str());

        m_zonePort              = rset->get<uint16>("zoneport");
        m_zoneMusic.m_songDay   = rset->get<uint8>("music_day");                      // background music (day)
        m_zoneMusic.m_songNight = rset->get<uint8>("music_night");                    // background music (night)
        m_zoneMusic.m_bSongS    = rset->get<uint8>("battlesolo");                     // solo battle music
        m_zoneMusic.m_bSongM    = rset->get<uint8>("battlemulti");                    // party battle music
        m_tax                   = static_cast<uint16>(rset->get<float>("tax") * 100); // tax for bazaar
        m_miscMask              = rset->get<uint16>("misc");

        m_zoneType = static_cast<ZONE_TYPE>(rset->get<uint32>("zonetype"));

        if (!rset->isNull("name")) // bcnmid cannot be used now, because they start from scratch
        {
