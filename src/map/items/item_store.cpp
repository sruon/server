...
auto ItemStore::destroy(const CItem* item) -> void
{
    ...
    // Anyone reaching the delete path with a still-claimed item has a
    // bug — the tx or binding needs to release ownership first. Loud
    // abort: silent delete here would corrupt tx accessors or leave
    // bindings pointing at freed memory.
    ...
    if (item->hasBinding())
    {
        ShowCriticalFmt("ItemStore::destroy: item {} is bound; caller must clear the binding first. Aborting.", item->getID());
        std::abort();
    }
    delete item;
}
...
auto ItemStore::placeInInventory(CItem* item, uint32_t charId, uint8_t location, uint8_t slot) -> void
{
    ...
    // Valid prior states: Unowned (fresh) or InCharContainer (re-home between
    // containers, e.g., recycle bin → inventory restore). Reject items
    // in a tx — they must be explicitly handed out by the tx.
    if (isInTransaction(item))
    {
...
auto ItemStore::reslotInventory(CItem* item, uint8_t newLocation, uint8_t newSlot) -> void
{
    ...
    // Only legal on items already InCharContainer — otherwise the caller
    // is trying to fabricate a home.
...
auto ItemStore::moveToTransaction(CItem* item, Transaction* tx) -> std::optional<ItemOwner>
{
    ...
    {
        // Only inventory-owned items can enter a fresh tx.
        return std::nullopt;
    }
...
auto ItemStore::adoptIntoTransaction(CItem* item, Transaction* tx) -> std::optional<ItemOwner>
{
    ...
    // Only legal on an Unowned item. Prevents silent owner-theft from
    // another inventory / tx.
    if (!std::holds_alternative<xi::item::Unowned>(item->owner()))
    {
        return std::nullopt;
    }
    writeOwner(item, xi::item::InTransaction{});
    // prior owner = Unowned; rollback will restore to Unowned, which
    // for a reconstructed item means "drop it" — the caller handles
    // cleanup through the tx's dtor/rollback.
...
auto ItemStore::isBusy(const CItem* item) -> bool
{
    return item != nullptr && (isInTransaction(item) || item->hasBinding());
}
...
auto ItemStore::consume(CCharEntity* owner, std::uint8_t location, std::uint8_t slot, std::uint32_t qty) -> std::uint16_t
{
    ...
    // Stack hit zero — destroy the CItem. Run the OnItemDrop Lua hook
    // and the style-locked fallback before the local unique_ptr drops.
    db::preparedStmt("DELETE FROM char_inventory WHERE charid = ? AND location = ? AND slot = ?",
                     owner->id,
                     location,
                     slot);
    auto droppedItem = container->ReleaseItem(slot);
    owner->pushPacket<GP_SERV_COMMAND_ITEM_ATTR>(nullptr, static_cast<CONTAINER_ID>(location), slot);

    // Style-locked fallback: if this was the last copy of a style-locked
    // weapon or piece of armor, revert the appearance so the client
    // doesn't keep rendering a ghost style with no backing item.
