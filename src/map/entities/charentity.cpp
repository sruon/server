...
bool CCharEntity::OnItemFinish(CItemState& state, action_t& action)
{
    TracyZoneScoped;
    ...
    // Trigger listener while item is still valid (before consumption)
    PAI->EventHandler.triggerListener("ITEM_USE", this, PItem, &action);
...
void CCharEntity::Die()
{
    ...
    if (auto* PLastAttacker = GetEntity(m_LastAttackerID.targid); PLastAttacker && PLastAttacker->id == m_LastAttackerID.id)
    {
