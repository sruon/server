...
{
    glz::schema str{ .description = "Inherits when omitted; the terminal fallback is c." };
    glz::schema dex{ .description = "Inherits when omitted; the terminal fallback is c." };
    glz::schema vit{ .description = "Inherits when omitted; the terminal fallback is c." };
    glz::schema agi{ .description = "Inherits when omitted; the terminal fallback is c." };
    glz::schema intelligence{ .description = "Inherits when omitted; the terminal fallback is c." };
    glz::schema mnd{ .description = "Inherits when omitted; the terminal fallback is c." };
    glz::schema chr{ .description = "Inherits when omitted; the terminal fallback is c." };
    glz::schema def{ .description = "Inherits when omitted; the terminal fallback is c." };
    glz::schema eva{ .description = "Inherits when omitted; the terminal fallback is c." };
    glz::schema att{ .description = "Inherits when omitted; the terminal fallback is a." };
    glz::schema acc{ .description = "Inherits when omitted; the terminal fallback is a." };
};

template <>
struct glz::json_schema<xi::data::datasets::ecosystems::wire::MobAttributes>
{
    glz::schema element{ .description = "Elemental affinity override. Omitted values inherit; the terminal fallback is none." };
    ...
    glz::schema detects{
        .description = "Detection mechanisms. Omitted values inherit, with an empty terminal fallback; redefine the entire list when overriding.",
        .uniqueItems = true,
    };
    glz::schema speed{
        .description = "Movement and animation speed override. Omitted values inherit, with a terminal fallback of 40; 0 is immobile.",
        .minimum     = 0L,
        .maximum     = 255L,
    };
    glz::schema charmable{ .description = "Whether the entity can be charmed. Omitted values inherit; the terminal fallback is false." };
};

template <>
struct glz::meta<xi::data::datasets::ecosystems::wire::EcosystemFile> : xi::data::yaml::DatasetRootSchema
{
    using T                     = xi::data::datasets::ecosystems::wire::EcosystemFile;
    static constexpr auto value = glz::object("meta", glz::skip{}, "ecosystems", &T::ecosystems);
};
...
struct glz::json_schema<xi::data::datasets::ecosystems::wire::Family>
{
    ...
    glz::schema species = xi::data::yaml::namedMapDoc("Species in this family, keyed by name. Names must be unique across all ecosystems.");
};
...
struct glz::json_schema<xi::data::datasets::ecosystems::wire::Ecosystem>
{
    ...
    glz::schema families = xi::data::yaml::namedMapDoc("Families in this ecosystem, keyed by name. Names must be unique across all ecosystems.");
};

template <>
struct glz::json_schema<xi::data::datasets::ecosystems::wire::EcosystemFile>
{
    glz::schema ecosystems = xi::data::yaml::namedMapDoc("Ecosystems keyed by name.");
};
