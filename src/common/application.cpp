...
void Application::tryIncreaseRLimits()
{
    ...
    // Raise the open-file (socket) soft limit as high as the hard limit allows.
    // Each connected player uses 3 sockets (auth/view/data); the previous fixed
    // 10240 capped xi_connect around 3400 players and wedged with "Too many open
    // files" under login churn. Take the full hard limit instead.
